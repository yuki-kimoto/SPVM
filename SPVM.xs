...
SPVM_OBJECT* SPVM_XS_UTIL_new_mulnum_array(SPVM_ENV* env, const char* basic_type_name, SV* sv_elems, SV** sv_error) {
  ...
  for (int32_t index = 0; index < length; index++) {
    ...
    if (sv_derived_from(sv_element, "HASH")) {
      ...
      SPVM_RUNTIME_CLASS* class = SPVM_API_get_runtime_class_from_basic_type_id(env, basic_type_id);
      assert(class);
      
      SPVM_RUNTIME_FIELD* mulnum_field = SPVM_API_get_runtime_field_from_index(env, class->id, 0);
      assert(mulnum_field);
      ...
      for (int32_t field_index = 0; field_index < class->field_ids_length; field_index++) {
        ...
        int32_t mulnum_field_type_basic_type_id = SPVM_API_get_type_basic_type_id(env, mulnum_field->type_id);
  ...
  PPCODE:
{
    ...
    switch (arg_type_category) {
      ...
      case SPVM_TYPE_C_TYPE_CATEGORY_MULNUM_BYTE: {
        if (sv_derived_from(sv_value, "HASH")) {
          SPVM_RUNTIME_BASIC_TYPE* arg_basic_type = SPVM_API_get_basic_type(env, arg_basic_type_id);
          SPVM_RUNTIME_CLASS* arg_class = SPVM_API_get_runtime_class_from_basic_type_id(env, arg_basic_type_id);
          HV* hv_value = (HV*)SvRV(sv_value);
          int32_t fields_length = arg_class->field_ids_length;
          for (int32_t field_index = 0; field_index < fields_length; field_index++) {
            ...
            if (sv_field_value_ptr) {
              ...
              croak("%dth argument's field \"%s\" of \"%s\" is missing at %s line %d\n", args_index_nth, field_name, SPVM_API_get_constant_string_value(env, arg_class->name_id, NULL), MFILE, __LINE__);
            }
            int8_t value = (int8_t)SvIV(sv_field_value);
            args_stack[args_stack_index + field_index].bval = value;
          }
          args_stack_index += arg_class->field_ids_length;
        }
        else {
          ...
          if (SvROK(sv_value) && sv_derived_from(sv_value, "ARRAY")) {
            ...
            if (arg_type_dimension == 1 || arg_basic_type_id == SPVM_BASIC_TYPE_C_ID_OARRAY) {
              switch (arg_basic_type_id) {
                ...
                default: {
                  if (arg_type_category == SPVM_TYPE_C_TYPE_CATEGORY_MULNUM_ARRAY) {
                    SV* sv_error = NULL;
                    SPVM_RUNTIME_BASIC_TYPE* arg_basic_type = SPVM_API_get_basic_type(env, arg_basic_type_id);
  ...
  switch (method_return_type_category) {
        ...
        if (return_value != NULL) {
          ...
          if (return_value->type_dimension > 0) {
            ...
            // Object
              ...
              sv_catpv(sv_perl_class_name, SPVM_API_get_basic_type_name(env, return_value->basic_type_id));
    ...
    case SPVM_TYPE_C_TYPE_CATEGORY_MULNUM_DOUBLE:
    {
      ...
      SPVM_RUNTIME_CLASS* method_return_class = SPVM_API_get_runtime_class_from_basic_type_id(env, method_return_basic_type_id);
      assert(method_return_class);
      
      SPVM_RUNTIME_FIELD* method_return_mulnum_field = SPVM_API_get_runtime_field_from_index(env, method_return_class->id, 0);
      assert(method_return_mulnum_field);
      ...
      for (int32_t field_index = 0; field_index < method_return_class->field_ids_length; field_index++) {
        ...
        int32_t method_return_mulnum_field_type_basic_type_id = SPVM_API_get_type_basic_type_id(env, method_return_mulnum_field->type_id);
    ...
    for (int32_t args_index = 0; args_index < method_arg_type_ids_length; args_index++) {
      ...
      switch (arg_type_category) {
        ...
        case SPVM_TYPE_C_TYPE_CATEGORY_REF_MULNUM_BYTE: {
          ...
          SPVM_RUNTIME_CLASS* arg_class = SPVM_API_get_runtime_class_from_basic_type_id(env, arg_basic_type_id);
          assert(arg_class);
          SPVM_RUNTIME_FIELD* arg_mulnum_field = SPVM_API_get_runtime_field_from_index(env, arg_class->id, 0);
  ...
  // Success
    ...
    if (method_return_type_category == SPVM_TYPE_C_TYPE_CATEGORY_VOID) {
      return_count = 0;
    }
    else {
  ...
  PPCODE:
{
  ...
  if (is_array_type) {
    ...
    if (array->type_category == SPVM_TYPE_C_TYPE_CATEGORY_MULNUM_ARRAY) {
      ...
      for (int32_t index = 0; index < length; index++) {
        SPVM_RUNTIME_CLASS* class = SPVM_API_get_runtime_class_from_basic_type_id(env, array->basic_type_id);
        assert(class);
        ...
        for (int32_t index = 0; index < length; index++) {
          ...
          if (value == NULL) {
            ...
            if (element_type_is_array_type) {
              ...
              SV* sv_perl_class_name = sv_2mortal(newSVpv("SPVM::", 0));
              sv_catpv(sv_perl_class_name, SPVM_API_get_basic_type_name(env, array->basic_type_id));
