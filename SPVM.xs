  ...
  for (int32_t index = 0; index < length; index++) {
    ...
    if (sv_derived_from(sv_element, "HASH")) {
      ...
      for (int32_t field_index = 0; field_index < class->field_ids->length; field_index++) {
        ...
        SPVM_RUNTIME_TYPE* first_field_type = SPVM_LIST_fetch(runtime_info->types, first_field->type_id);
  ...
  PPCODE:
{
  ...
  for (int32_t args_index = 0; args_index < method->arg_type_ids->length; args_index++) {
    ...
    SPVM_RUNTIME_TYPE* arg_type = SPVM_LIST_fetch(runtime_info->types, arg_type_id);
  ...
  switch (method_return_type->category) {
    ...
    case SPVM_TYPE_C_TYPE_CATEGORY_MULNUM_DOUBLE:
    {
      ...
      SPVM_RUNTIME_BASIC_TYPE* method_return_basic_type = SPVM_LIST_fetch(runtime_info->basic_types, method_return_basic_type_id);
      ...
      for (int32_t field_index = 0; field_index < method_return_class->field_ids->length; field_index++) {
        ...
        SPVM_RUNTIME_TYPE* method_return_first_field_type = SPVM_LIST_fetch(runtime_info->types, method_return_first_field->type_id);
  ...
  PPCODE:
{
  ...
  if (is_array_type) {
    ...
    if (array->type_category == SPVM_TYPE_C_TYPE_CATEGORY_MULNUM_ARRAY) {
      ...
      for (int32_t index = 0; index < length; index++) {
        SPVM_RUNTIME_BASIC_TYPE* basic_type = SPVM_LIST_fetch(runtime_info->basic_types, array->basic_type_id);
  ...
  PPCODE:
{
  ...
  // Name
  const char* class_name = SvPV_nolen(sv_class_name);
  char* class_name_copy = SPVM_ALLOCATOR_new_block_permanent(compiler->allocator, sv_len(sv_class_name) + 1);
  memcpy(class_name_copy, class_name, sv_len(sv_class_name));
  
  // File
  const char* start_file = SvPV_nolen(sv_start_file);
  char* start_file_copy = SPVM_ALLOCATOR_new_block_permanent(compiler->allocator, sv_len(sv_start_file) + 1);
