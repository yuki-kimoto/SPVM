  ...
  PPCODE:
{
  ...
  int32_t die_error_id_ret = env->call_method_raw(env, stack, method_id, args_native_stack_length);
  
  if (die_error_id_ret) {
    if (SvOK(sv_error_ret)) {
      HV* hv_error_ret = (HV*)SvRV(sv_error_ret);
      SV* sv_die_error_id_ret = sv_2mortal(newSViv(die_error_id_ret));
      (void)hv_store(hv_error_ret, "code", strlen("code"), SvREFCNT_inc(sv_die_error_id_ret), 0);
    }
  ...
  PPCODE:
{
  ...
  // Compile SPVM
  int32_t compile_die_error_id = api_env->api->compiler->compile(compiler, class_name);
