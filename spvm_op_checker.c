...
void SPVM_OP_CHECKER_check(SPVM_COMPILER* compiler) {
  ...
  // Resolve String type
  SPVM_TYPE* type_string = SPVM_LIST_fetch(compiler->types, SPVM_TYPE_C_CODE_STRING);
  ...
  
  {
    ...
    for (sub_pos = 0; sub_pos < compiler->op_subs->length; sub_pos++) {
      ...
      int32_t loop_structure_count = 0;
            ...
            while (1) {
              ...
              switch (op_cur->code) {
                ...
                case SPVM_OP_C_CODE_LOOP: {
                  ...
                  op_cur->uv.loop_block_index = loop_structure_count;
                  loop_structure_count++;
      ...
      sub->loop_structure_count = loop_structure_count;
