...
sub get_dependent_resources {
  ...
  for my $class_name (@$class_names_without_anon) {
    ...
    my $native_method_names = $builder->get_method_names($builder->runtime, $class_name, 'native');
    if (@$native_method_names) {
      my $native_module_file = $builder->get_module_file($builder->runtime, $class_name);
      ...
      # Module file
      my $module_file = $builder->get_module_file($builder->runtime, $class_name);
...
EOS
  ...
  unless ($no_precompile) {
    ...
    for my $class_name (@$class_names) {
      my $precompile_method_names = $builder->get_method_names($builder->runtime, $class_name, 'precompile');
...
EOS
  ...
  for my $class_cname (@$class_names_without_anon) {
    my $native_method_names = $builder->get_method_names($builder->runtime, $class_cname, 'native');
