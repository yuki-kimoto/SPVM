...
sub get_dependent_resources {
  ...
  for my $class_name (@$class_names_without_anon) {
    ...
    my $native_method_names = SPVM::Builder::Runtime->get_method_names($builder->runtime, $class_name, 'native');
...
EOS
  ...
  unless ($no_precompile) {
    ...
    for my $class_name (@$class_names) {
      my $precompile_method_names = SPVM::Builder::Runtime->get_method_names($builder->runtime, $class_name, 'precompile');
...
EOS
  ...
  for my $class_cname (@$class_names_without_anon) {
    my $native_method_names = SPVM::Builder::Runtime->get_method_names($builder->runtime, $class_cname, 'native');
