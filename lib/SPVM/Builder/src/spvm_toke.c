      ...
      case '\'': {
        ...
        if (*compiler->bufptr == '\'') {
          ...
          if (*compiler->bufptr == '\'') {
            ...
            SPVM_COMPILER_error(compiler, "Can't find the terminiator \"'\" of the character literal at %s line %d", compiler->cur_file, compiler->cur_line);
          }
        }
      ...
      default:
        ...
        // Keyword or name
            ...
            switch (keyword[0]) {
              ...
              case 'd' : {
                if (strcmp(keyword, "default") == 0) {
                  ...
                  yylvalp->opval = SPVM_TOKE_newOP(compiler, SPVM_OP_C_ID_DIV_UINT);
                  return DIV_UINT;
                }
                else if (strcmp(keyword, "div_ulong") == 0) {
                  yylvalp->opval = SPVM_TOKE_newOP(compiler, SPVM_OP_C_ID_DIV_ULONG);
                  return DIV_ULONG;
                }
              ...
              case 'r' : {
                if (strcmp(keyword, "ref") == 0) {
                  ...
                  yylvalp->opval = SPVM_TOKE_newOP(compiler, SPVM_OP_C_ID_REM_UINT);
                  return REM_UINT;
                }
                else if (strcmp(keyword, "rem_ulong") == 0) {
                  yylvalp->opval = SPVM_TOKE_newOP(compiler, SPVM_OP_C_ID_REM_ULONG);
                  return REM_ULONG;
                }
          ...
          if (strstr(keyword, "__")) {
            SPVM_COMPILER_error(compiler, "Symbol name \"%s\" must not contains __ at %s line %d", keyword, compiler->cur_file, compiler->cur_line);
          }
