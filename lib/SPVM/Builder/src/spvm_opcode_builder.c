                            ...
                            // To object type
                              ...
                              if (SPVM_TYPE_is_object_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                                ...
                                int32_t runtime_assignability = SPVM_TYPE_can_assign(
                                  ...
                                  NULL, &need_implicite_conversion, &narrowing_conversion_error, &mutable_invalid, &narrowing_conversion
                                );
...
SPVM_OP* SPVM_OPCODE_BUILDER_get_target_op_var(SPVM_COMPILER* compiler, SPVM_OP* op) {
  ...
  if (op->id == SPVM_OP_C_ID_VAR) {
    ...
    if (op->first->id == SPVM_OP_C_ID_VAR || op->first->id == SPVM_OP_C_ID_ASSIGN) {
      ...
      op_var = SPVM_OPCODE_BUILDER_get_target_op_var(compiler, op->last);
    }
    else {
      assert(0);
    }
