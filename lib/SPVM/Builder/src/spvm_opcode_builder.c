...
)
{
  ...
  if (push_eval_opcode_rel_index_stack->length > 0) {
    ...
    int32_t if_die_catch_opcode_rel_index = opcode_array->length - 1 - method_opcodes_base;
    ...
    int32_t if_die_return_opcode_rel_index = opcode_array->length - 1 - method_opcodes_base;
...
void SPVM_OPCODE_BUILDER_build_opcode_array(SPVM_COMPILER* compiler) {
  ...
  
  {
    ...
    for (package_index = compiler->cur_package_base; package_index < compiler->packages->length; package_index++) {
      ...
      SPVM_LIST* methods = package->methods;
      {
        ...
        for (method_index = 0; method_index < methods->length; method_index++) {
          ...
          assert(method->package->module_file);
          ...
          method->opcodes_base = method_opcodes_base;
            ...
            switch (op_cur->id) {
              case SPVM_OP_C_ID_BLOCK: { // Preorder
                ...
                if (op_cur->uv.block->id == SPVM_BLOCK_C_ID_LOOP_INIT) {
                  ...
                  int32_t opcode_rel_index = opcode_array->length - 1 - method_opcodes_base;
                  ...
                  int32_t opcode_rel_index = opcode_array->length - method_opcodes_base;
              ...
              case SPVM_OP_C_ID_DEFAULT: {
                if (switch_info_stack->length > 0) {
                  SPVM_SWITCH_INFO* switch_info = SPVM_LIST_fetch(switch_info_stack, switch_info_stack->length - 1);
                  switch_info->default_opcode_rel_index = opcode_array->length - method_opcodes_base;
                }
                break;
              }
            }
                ...
                switch (op_cur->id) {
                  case SPVM_OP_C_ID_BLOCK: { // Postorder
                    if (op_cur->uv.block->id == SPVM_BLOCK_C_ID_IF) {
                      ...
                      // Set jump
                      SPVM_OPCODE* if_eq_or_if_ne_goto = (opcode_array->values + method_opcodes_base + if_eq_or_if_ne_goto_opcode_rel_index);
                      int32_t if_eq_or_if_ne_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      SPVM_OPCODE* if_block_end_goto = (opcode_array->values + method_opcodes_base + if_block_end_goto_opcode_rel_index);
                      int32_t if_block_end_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      for (int32_t i = 0; i < last_goto_opcode_rel_index_stack_pop_count; i++) {
                        ...
                        SPVM_OPCODE* last_goto = (opcode_array->values + method_opcodes_base + last_goto_opcode_rel_index);
                        int32_t last_goto_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      for (int32_t i = 0; i < next_goto_opcode_rel_index_stack_pop_count; i++) {
                        ...
                        SPVM_OPCODE* next_goto = (opcode_array->values + method_opcodes_base + next_goto_opcode_rel_index);
                        int32_t next_goto_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      for (int32_t i = 0; i < break_goto_opcode_rel_index_stack_pop_count; i++) {
                        ...
                        SPVM_OPCODE* break_goto = (opcode_array->values + method_opcodes_base + break_goto_opcode_rel_index);
                        int32_t break_goto_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      while (if_die_catch_goto_opcode_rel_index_stack->length > 0) {
                        ...
                        SPVM_OPCODE* if_die_catch_goto = (opcode_array->values + method_opcodes_base + if_die_catch_goto_opcode_rel_index);
                        int32_t if_die_catch_goto_jump_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      while (return_goto_opcode_rel_index_stack->length > 0) {
                        ...
                        SPVM_OPCODE* return_goto = (opcode_array->values + method_opcodes_base + return_goto_opcode_rel_index);
                        int32_t return_goto_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      while (if_die_return_goto_opcode_rel_index_stack->length > 0) {
                        ...
                        SPVM_OPCODE* if_die_return_goto = (opcode_array->values + method_opcodes_base + if_die_return_goto_opcode_rel_index);
                        int32_t if_die_return_goto_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                  ...
                  case SPVM_OP_C_ID_ASSIGN: {
                    ...
                    if (op_assign_dist->id == SPVM_OP_C_ID_VAR) {
                      ...
                      switch (op_assign_src->id) {
                        case SPVM_OP_C_ID_ASSIGN: {
                          ...
                          if (SPVM_TYPE_is_byte_type(compiler, type_dist->basic_type->id, type_dist->dimension, type_dist->flag)) {
                            ...
                            switch (field_type->basic_type->id) {
                              case SPVM_BASIC_TYPE_C_ID_BYTE: {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_MOVE_BYTE);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                                break;
                              }
                              case SPVM_BASIC_TYPE_C_ID_SHORT: {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_MOVE_SHORT);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                                break;
                              }
                              case SPVM_BASIC_TYPE_C_ID_INT: {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_MOVE_INT);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                                break;
                              }
                              case SPVM_BASIC_TYPE_C_ID_LONG: {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_MOVE_LONG);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                                break;
                              }
                              case SPVM_BASIC_TYPE_C_ID_FLOAT: {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_MOVE_FLOAT);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                                break;
                              }
                              case SPVM_BASIC_TYPE_C_ID_DOUBLE: {
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_MOVE_DOUBLE);
                                break;
                              }
                              default:
                                assert(0);
                        ...
                        case SPVM_OP_C_ID_CALL_METHOD: {
                          ...
                          SPVM_CALL_METHOD* call_spvm_method = op_assign_src->uv.call_spvm_method;
                          ...
                          SPVM_METHOD* method_call_spvm_method = SPVM_HASH_fetch(call_spvm_method_method_package->method_symtable, call_spvm_method_method_name, strlen(call_spvm_method_method_name));
                          
                          int32_t first_arg_mem_id = -1;
                          SPVM_OP* op_term_args = op_assign_src->last;
                          SPVM_OP* op_term_arg = op_term_args->first;

                          SPVM_LIST* args = method_call_spvm_method->args;
                          {
                            int32_t arg_index;
                            for (arg_index = 0; arg_index < method_call_spvm_method->args->length; arg_index++) {
                          ...
                          SPVM_TYPE* call_spvm_method_return_type = call_spvm_method->method->return_type;
                          if (method_call_spvm_method->package->category == SPVM_PACKAGE_C_CATEGORY_CALLBACK) {
                            // Numeric type
                            if (SPVM_TYPE_is_numeric_type(compiler, call_spvm_method_return_type->basic_type->id, call_spvm_method_return_type->dimension, call_spvm_method_return_type->flag)) {
                              switch (call_spvm_method_return_type->basic_type->id) {
                                case SPVM_BASIC_TYPE_C_ID_BYTE: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_BYTE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_SHORT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_SHORT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_INT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_INT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_LONG: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_LONG);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_FLOAT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_FLOAT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_DOUBLE: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_DOUBLE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                default:
                            ...
                            // void type
                            else if (SPVM_TYPE_is_void_type(compiler, call_spvm_method_return_type->basic_type->id, call_spvm_method_return_type->dimension, call_spvm_method_return_type->flag)) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_VOID);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            // Object type
                            else if (SPVM_TYPE_is_object_type(compiler, call_spvm_method_return_type->basic_type->id, call_spvm_method_return_type->dimension, call_spvm_method_return_type->flag)) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_OBJECT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            // Multi numeric type
                            else if (SPVM_TYPE_is_multi_numeric_type(compiler, call_spvm_method_return_type->basic_type->id, call_spvm_method_return_type->dimension, call_spvm_method_return_type->flag)) {

                              SPVM_PACKAGE* value_package = call_spvm_method_return_type->basic_type->package;
                              assert(package);
                              ...
                              switch (field_type->basic_type->id) {
                                case SPVM_BASIC_TYPE_C_ID_BYTE: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_MULNUM_BYTE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_SHORT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_MULNUM_SHORT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_INT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_MULNUM_INT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_LONG: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_MULNUM_LONG);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_FLOAT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_MULNUM_FLOAT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_DOUBLE: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD_MULNUM_DOUBLE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                default:
                            ...
                            opcode.operand0 = mem_id_out;
                            opcode.operand1 = call_spvm_method->method->id;
                            ...
                            // Multi numeric type
                              ...
                              switch (field_type->basic_type->id) {
                                case SPVM_BASIC_TYPE_C_ID_BYTE: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_METHOD_MULNUM_BYTE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_SHORT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_METHOD_MULNUM_SHORT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_INT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_METHOD_MULNUM_INT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_LONG: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_METHOD_MULNUM_LONG);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_FLOAT: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_METHOD_MULNUM_FLOAT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                case SPVM_BASIC_TYPE_C_ID_DOUBLE: {
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_METHOD_MULNUM_DOUBLE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                }
                                default:
                          ...
                          int32_t call_spvm_method_return_type_width = SPVM_TYPE_get_width(compiler, call_spvm_method_return_type->basic_type->id, call_spvm_method_return_type->dimension, call_spvm_method_return_type->flag);
                          opcode.operand3 = call_spvm_method_return_type_width;
                        ...
                        case SPVM_OP_C_ID_ARRAY_FIELD_ACCESS : {
                          ...
                          switch (element_type->basic_type->id) {
                            case SPVM_BASIC_TYPE_C_ID_BYTE:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FIELD_FETCH_BYTE);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_SHORT:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FIELD_FETCH_SHORT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_INT:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FIELD_FETCH_INT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_LONG:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FIELD_FETCH_LONG);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_FLOAT:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FIELD_FETCH_FLOAT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_DOUBLE:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FIELD_FETCH_DOUBLE);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                              break;
                            default:
                              assert(0);
                        ...
                        case SPVM_OP_C_ID_FIELD_ACCESS : {
                          ...
                          if (op_assign_src->flag & SPVM_OP_C_FLAG_FIELD_ACCESS_WEAKEN) {
                            ...
                            if (SPVM_TYPE_is_value_ref_type(compiler, invocant_type->basic_type->id, invocant_type->dimension, invocant_type->flag)) {
                              ...
                              switch (field_type->basic_type->id) {
                                case SPVM_BASIC_TYPE_C_ID_BYTE:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_GET_FIELD_BYTE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_SHORT:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_GET_FIELD_SHORT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_INT:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_GET_FIELD_INT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_LONG:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_GET_FIELD_LONG);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_FLOAT:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_GET_FIELD_FLOAT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_DOUBLE:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_GET_FIELD_DOUBLE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  break;
                                default:
                                  assert(0);
                            ...
                            // Value field access
                              ...
                              switch (field_type->basic_type->id) {
                                case SPVM_BASIC_TYPE_C_ID_BYTE:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_GET_FIELD_BYTE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  mem_id_in = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_SHORT:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_GET_FIELD_SHORT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  mem_id_in = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_INT:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_GET_FIELD_INT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  mem_id_in = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_LONG:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_GET_FIELD_LONG);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  mem_id_in = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_FLOAT:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_GET_FIELD_FLOAT);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  mem_id_in = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                                  break;
                                case SPVM_BASIC_TYPE_C_ID_DOUBLE:
                                  SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_GET_FIELD_DOUBLE);
                                  mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                  mem_id_in = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                                  break;
                                default:
                        ...
                        case SPVM_OP_C_ID_ARRAY_ACCESS : {
                          ...
                          if (SPVM_TYPE_is_value_array_type(compiler, array_type->basic_type->id, array_type->dimension, array_type->flag)) {
                            ...
                            switch (element_type->basic_type->id) {
                              case SPVM_BASIC_TYPE_C_ID_BYTE:
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FETCH_BYTE);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                break;
                              case SPVM_BASIC_TYPE_C_ID_SHORT:
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FETCH_SHORT);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                break;
                              case SPVM_BASIC_TYPE_C_ID_INT:
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FETCH_INT);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                break;
                              case SPVM_BASIC_TYPE_C_ID_LONG:
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FETCH_LONG);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                break;
                              case SPVM_BASIC_TYPE_C_ID_FLOAT:
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FETCH_FLOAT);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                break;
                              case SPVM_BASIC_TYPE_C_ID_DOUBLE:
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_FETCH_DOUBLE);
                                mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                                break;
                              default:
                                assert(0);
                        ...
                        case SPVM_OP_C_ID_DEREF : {
                          ...
                          if (SPVM_TYPE_is_multi_numeric_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_BYTE) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_BYTE);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_SHORT) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SHORT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_INT) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_INT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_LONG) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_LONG);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_FLOAT) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_FLOAT);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_DOUBLE) {
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_DOUBLE);
                              mem_id_out = SPVM_OP_get_mem_id(compiler, op_assign_dist);
                            }
                            else {
                              assert(0);
                        ...
                        case SPVM_OP_C_ID_METHODTRACT : {
                          ...
                          switch (type_dist->basic_type->id) {
                            case SPVM_BASIC_TYPE_C_ID_INT:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_METHODTRACT_INT);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_LONG:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_METHODTRACT_LONG);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_FLOAT:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_METHODTRACT_FLOAT);
                              break;
                            case SPVM_BASIC_TYPE_C_ID_DOUBLE:
                              SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_METHODTRACT_DOUBLE);
                              break;
                            default:
                              assert(0);
                          }
                        ...
                        case SPVM_OP_C_ID_NUMERIC_EQ : {
                          if (op_assign_src->first->id == SPVM_OP_C_ID_UNDEF || op_assign_src->last->id == SPVM_OP_C_ID_UNDEF) {
                            ...
                            if (SPVM_TYPE_is_numeric_type(compiler, operand_type->basic_type->id, operand_type->dimension, operand_type->flag)) {
                              if (operand_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_BYTE) {
                                assert(0);
                              }
                              else if (operand_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_SHORT) {
                                assert(0);
                              }
                              else if (operand_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_INT) {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_EQ_INT);
                                mem_id_in1 = SPVM_OP_get_mem_id(compiler, op_assign_src->first);
                                mem_id_in2 = SPVM_OP_get_mem_id(compiler, op_assign_src->last);
                              }
                              else if (operand_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_LONG) {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_EQ_LONG);
                                mem_id_in1 = SPVM_OP_get_mem_id(compiler, op_assign_src->first);
                                mem_id_in2 = SPVM_OP_get_mem_id(compiler, op_assign_src->last);
                              }
                              else if (operand_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_FLOAT) {
                                SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_EQ_FLOAT);
                                mem_id_in1 = SPVM_OP_get_mem_id(compiler, op_assign_src->first);
                                mem_id_in2 = SPVM_OP_get_mem_id(compiler, op_assign_src->last);
                              }
                              else if (operand_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_DOUBLE) {
                        ...
                        case SPVM_OP_C_ID_RETURN: {
                          if (op_assign_src->first) {
                            ...
                            if (SPVM_TYPE_is_undef_type(compiler, return_type->basic_type->id, return_type->dimension, return_type->flag)) {
                              ...
                              else if (SPVM_TYPE_is_multi_numeric_type(compiler, method->return_type->basic_type->id, method->return_type->dimension, method->return_type->flag)) {
                          ...
                          int32_t return_opcode_rel_index = opcode_array->length - 1 - method_opcodes_base;
                        ...
                        case SPVM_OP_C_ID_LOOP_INCREMENT: {
                          ...
                          SPVM_OPCODE* loop_first_goto = (opcode_array->values + method_opcodes_base + loop_first_goto_opcode_rel_index);
                          int32_t loop_first_goto_jump_opcode_rel_index = opcode_array->length - method_opcodes_base;
                      ...
                      if (SPVM_TYPE_is_oarray_type(compiler, array_type->basic_type->id, array_type->dimension, array_type->flag)) {
                        ...
                        int32_t mem_id_in;
                        if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_BYTE) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_STORE_BYTE);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_SHORT) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_STORE_SHORT);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_INT) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_STORE_INT);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_LONG) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_STORE_LONG);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_FLOAT) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_STORE_FLOAT);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (element_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_DOUBLE) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_ARRAY_STORE_DOUBLE);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else {
                          assert(0);
                    ...
                    // Set field
                      ...
                      if (SPVM_TYPE_is_value_ref_type(compiler, invocant_type->basic_type->id, invocant_type->dimension, invocant_type->flag)) {
                        ...
                        switch (src_type->basic_type->id) {
                          case SPVM_BASIC_TYPE_C_ID_BYTE:
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SET_FIELD_BYTE);
                            mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                            break;
                          case SPVM_BASIC_TYPE_C_ID_SHORT:
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SET_FIELD_SHORT);
                            mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                            break;
                          case SPVM_BASIC_TYPE_C_ID_INT:
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SET_FIELD_INT);
                            mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                            break;
                          case SPVM_BASIC_TYPE_C_ID_LONG:
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SET_FIELD_LONG);
                            mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                            break;
                          case SPVM_BASIC_TYPE_C_ID_FLOAT:
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SET_FIELD_FLOAT);
                            mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                            break;
                          case SPVM_BASIC_TYPE_C_ID_DOUBLE:
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_DEREF_SET_FIELD_DOUBLE);
                            mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                            break;
                          default:
                            assert(0);
                        ...
                        if (src_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_BYTE) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_SET_FIELD_BYTE);
                          mem_id_out = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (src_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_SHORT) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_SET_FIELD_SHORT);
                          mem_id_out = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (src_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_INT) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_SET_FIELD_INT);
                          mem_id_out = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (src_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_LONG) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_SET_FIELD_LONG);
                          mem_id_out = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (src_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_FLOAT) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_SET_FIELD_FLOAT);
                          mem_id_out = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else if (src_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_DOUBLE) {
                          SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_MULNUM_SET_FIELD_DOUBLE);
                          mem_id_out = SPVM_OP_get_mem_id(compiler, op_term_invocant);
                          mem_id_in = SPVM_OP_get_mem_id(compiler, op_assign_src);
                        }
                        else {
          ...
          method->opcodes_length = opcode_array->length - method->opcodes_base;
