...
void SPVM_OPCODE_BUILDER_build_opcodes(SPVM_COMPILER* compiler) {
  ...
  for (int32_t basic_type_id = compiler->basic_types_base_id; basic_type_id < compiler->basic_types->length; basic_type_id++) {
    ...
    for (int32_t method_index = 0; method_index < methods->length; method_index++) {
      ...
      SPVM_LIST* block_stack_typed_var_index_top_v2 = SPVM_LIST_new(compiler->current_each_compile_allocator, 0, SPVM_ALLOCATOR_C_ALLOC_TYPE_TMP);
      ...
      SPVM_LIST_free(block_stack_typed_var_index_top_v2);
      ...
      END_OF_FUNCTION: {
        ...
        // At least 1 element
        method->mortal_stack_length_v2 = new_object_count + 1;
