    ...
    for (class_index = compiler->cur_class_base; class_index < compiler->classes->length; class_index++) {
      ...
      SPVM_LIST* methods = class->methods;
      {
        ...
        for (method_index = 0; method_index < methods->length; method_index++) {
          ...
          SPVM_LIST* if_block_end_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);
          
          // opcode index stack for loop start
          SPVM_LIST* loop_first_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);
          
          // opcode index stack for last
          SPVM_LIST* last_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);

          // opcode index stack for break
          SPVM_LIST* break_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);
          
          // opcode index stack for next
          SPVM_LIST* next_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);
          
          // opcode index stack for eval start
          SPVM_LIST* push_eval_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);
          
          // IF_EXCEPTION_CATCH opcode index stack
          SPVM_LIST* if_die_catch_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);

          // IF_EXCEPTION_RETURN opcode index stack
          SPVM_LIST* if_die_return_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);

          // RETURN goto opcode index stack
          SPVM_LIST* return_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0, 0);
          ...
          SPVM_LIST* last_block_base_stack = SPVM_LIST_new(compiler, 0, 0);
          ...
          SPVM_LIST* mortal_top_stack = SPVM_LIST_new(compiler, 0, 0);
