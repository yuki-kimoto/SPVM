            ...
            switch (op_cur->id) {
              case SPVM_OP_C_ID_BLOCK: { // Postorder
                ...
                if (block->id == SPVM_BLOCK_C_ID_EVAL) {
                  ...
                  while (exception_catch_info_opcode_index_stack->length > 0) {
                    int32_t exception_catch_info_goto_opcode_index = (intptr_t)SPVM_LIST_pop(exception_catch_info_opcode_index_stack);
                    SPVM_OPCODE* exception_catch_info_goto = (exception_catch_info_opcodes_list->values + exception_catch_info_goto_opcode_index);
                    // Operand 2: end of eval block opcode_index.
                    exception_catch_info_goto->operand2 = opcode_list->length;
                  }
                  ...
                  switch (op_assign_src->id) {
                    ...
                    case SPVM_OP_C_ID_CALL_METHOD: {
                      ...
                      // Call method
                      {
                        ...
                        // Operand 0: Basic type ID
                        opcode.operand0 = call_method->method->current_basic_type->id;
                        // Operand 1: Method index
                        opcode.operand1 = call_method->method->index;
                        
                        // Operand 2: Line number (int32_t)
                        opcode.operand2 = call_method->line;
                        
                        // Operand 3: Arguments width (uint16_t)
                        // Check if args_type_width does not exceed the maximum value of uint16_t
                        assert(args_type_width <= 0xFFFF);
                        opcode.operand3 = (uint16_t)args_type_width;
                      ...
                      int32_t no_die = 1;
                      SPVM_OPCODE_BUILDER_push_opcode_on_exception(compiler, opcode_list, eval_block_stack->length, catch_on_exception_opcode_index_stack, return_on_exception_opcode_index_stack, method->op_method, op_cur->line, no_die, exception_catch_info_opcodes_list, exception_catch_info_opcode_index_stack);
                    ...
                    case SPVM_OP_C_ID_TYPE_CAST : {
                      ...
                      if (SPVM_TYPE_is_undef_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                        ...
                        if (SPVM_TYPE_is_object_type(compiler, cast_type->basic_type->id, cast_type->dimension, cast_type->flag)) {
                          ...
                          SPVM_FIELD* first_field = SPVM_LIST_get(cast_type->basic_type->fields, 0);
                          assert(first_field);
                          ...
                          int32_t fields_length = cast_type->basic_type->fields->length;
                        ...
                        if (SPVM_TYPE_is_byte_type(compiler, cast_type->basic_type->id, cast_type->dimension, cast_type->flag)) {
                          if (SPVM_TYPE_is_byte_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_BYTE);
                            check_exception = 1;
                          }
                        ...
                        // To short
                          ...
                          if (SPVM_TYPE_is_byte_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_SHORT);
                            check_exception = 1;
                          }
                          else if (SPVM_TYPE_is_any_object_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_SHORT);
                        ...
                        // To int
                          ...
                          if (SPVM_TYPE_is_byte_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_INT);
                            check_exception = 1;
                          }
                          else if (SPVM_TYPE_is_any_object_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_INT);
                        ...
                        // To long
                          ...
                          if (SPVM_TYPE_is_byte_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_LONG);
                            check_exception = 1;
                          }
                          else if (SPVM_TYPE_is_any_object_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_LONG);
                        ...
                        // To float
                          ...
                          if (SPVM_TYPE_is_byte_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_FLOAT);
                            check_exception = 1;
                          }
                          else if (SPVM_TYPE_is_any_object_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_FLOAT);
                        ...
                        // To double
                          ...
                          if (SPVM_TYPE_is_byte_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            ...
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_DOUBLE);
                            check_exception = 1;
                          }
                          else if (SPVM_TYPE_is_any_object_type(compiler, src_type->basic_type->id, src_type->dimension, src_type->flag)) {
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_TYPE_CONVERSION_ANY_OBJECT_TO_DOUBLE);
...
void SPVM_OPCODE_BUILDER_push_opcode_on_exception(
  ...
  SPVM_LIST* exception_catch_info_opcode_index_stack
)
{
