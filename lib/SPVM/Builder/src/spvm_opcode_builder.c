    ...
    for (class_index = compiler->cur_class_base; class_index < compiler->classes->length; class_index++) {
      ...
      SPVM_LIST* methods = class->methods;
      {
        ...
        for (method_index = 0; method_index < methods->length; method_index++) {
          ...
          SPVM_LIST* if_eq_or_if_ne_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // opcode index stack for if end
          SPVM_LIST* if_block_end_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // opcode index stack for loop start
          SPVM_LIST* loop_first_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // opcode index stack for last
          SPVM_LIST* last_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);

          // opcode index stack for break
          SPVM_LIST* break_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // opcode index stack for next
          SPVM_LIST* next_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // opcode index stack for eval start
          SPVM_LIST* push_eval_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // IF_EXCEPTION_CATCH opcode index stack
          SPVM_LIST* if_die_catch_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);

          // IF_EXCEPTION_RETURN opcode index stack
          SPVM_LIST* if_die_return_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);

          // RETURN goto opcode index stack
          SPVM_LIST* return_goto_opcode_rel_index_stack = SPVM_LIST_new(compiler, 0);
          
          // Switch stack
          SPVM_LIST* switch_info_stack = SPVM_LIST_new(compiler, 0);
          
          // next block base stack
          SPVM_LIST* next_block_base_stack = SPVM_LIST_new(compiler, 0);
          
          // last block base stack
          SPVM_LIST* last_block_base_stack = SPVM_LIST_new(compiler, 0);

          // break block base stack
          SPVM_LIST* break_block_base_stack = SPVM_LIST_new(compiler, 0);
          
          // Block stack
          SPVM_LIST* op_block_stack = SPVM_LIST_new(compiler, 0);
          
          // Mortal variable stack
          SPVM_LIST* mortal_stack = SPVM_LIST_new(compiler, 0);
          
          // Mortal variable base stack
          SPVM_LIST* mortal_top_stack = SPVM_LIST_new(compiler, 0);

          // Object temporary variable stack
          SPVM_LIST* object_op_var_tmp_stack = SPVM_LIST_new(compiler, 0);
                      ...
                      switch (op_assign_src->id) {
                        ...
                        case SPVM_OP_C_ID_CALL_METHOD: {
                          ...
                          if (method_call_method->class->category == SPVM_CLASS_C_CATEGORY_CALLBACK) {
                            SPVM_OPCODE_BUILDER_set_opcode_id(compiler, &opcode, SPVM_OPCODE_C_ID_CALL_CALLBACK_METHOD);
