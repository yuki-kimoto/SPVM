...
SPVM_OPCODE_ARRAY* SPVM_OPCODE_ARRAY_new(SPVM_COMPILER* compiler) {
  ...
  SPVM_OPCODE_ARRAY* opcodes = SPVM_ALLOCATOR_new_block_compile_eternal(compiler->allocator, sizeof(SPVM_OPCODE_ARRAY));
  opcodes->capacity = 64;
  opcodes->length = 0;
  
  int64_t values_byte_size = (int64_t)opcodes->capacity * (int64_t)sizeof(SPVM_OPCODE);
  SPVM_OPCODE* values = SPVM_ALLOCATOR_new_block_compile_eternal(compiler->allocator, values_byte_size);
...
void SPVM_OPCODE_ARRAY_push_opcode(SPVM_COMPILER* compiler, SPVM_OPCODE_ARRAY* opcodes, SPVM_OPCODE* opcode) {
  ...
  if (length >= capacity) {
    ...
    SPVM_OPCODE* new_values = SPVM_ALLOCATOR_new_block_compile_eternal(compiler->allocator, new_values_byte_size);
