...
void SPVM_API_call_destroy_method(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* obj_invocant, SPVM_RUNTIME_METHOD* destroy_method, const char* func_name, const char* file, int32_t line) {
  ...
  stack[0].oval = (SPVM_OBJ*)obj_invocant;
...
void SPVM_API_warn_common(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* string, const char* func_name, const char* file, int32_t line, int32_t longmess) {
  ...
  if (longmess) {
    ...
    int32_t current_call_depth = stack[SPVM_API_C_STACK_INDEX_CALL_DEPTH].ival;
    SPVM_OBJECT* obj_longmess = SPVM_API_longmess(env, stack, obj_empty_string, func_name, file, line);
    ...
    if (!do_dec_ref_count_only) {
      ...
      // Free released_object
        ...
        if (released_object_basic_type_category == SPVM_NATIVE_C_BASIC_TYPE_CATEGORY_CLASS) {
          ...
          if (destroy_method) {
            SPVM_API_call_destroy_method(env, stack, released_object, destroy_method, __func__, FILE_NAME, __LINE__);
          }
...
SPVM_OBJECT* SPVM_API_longmess(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* obj_message, const char* func_name, const char* file, int32_t line) {
  SPVM_OBJECT* obj_longmess = SPVM_API_longmess_no_mortal(env, stack, obj_message, func_name, file, line);
