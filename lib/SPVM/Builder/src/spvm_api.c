...
int32_t SPVM_API_call_instance_method_common(SPVM_ENV* env, SPVM_VALUE* stack, const char* method_name, int32_t args_width, int32_t mortal, int32_t check_args_level, const char* decl_args_signature, const char* func_name, const char* file, int32_t line) {
  ...
  SPVM_OBJECT* object = (SPVM_OBJECT*)stack[0].oval;
  ...
  if (__builtin_expect(!!object, 1)) {
    ...
    if (__builtin_expect(!!method, 1)) {
      ...
      SPVM_OBJECT* obj_invocant_type_name = SPVM_API_get_type_name(env, stack, object);
      const char* invocant_type_name = SPVM_API_get_chars(env, stack, obj_invocant_type_name);
      ...
      SPVM_OBJECT* exception = SPVM_API_new_string_nolen_no_mortal(env, stack, tmp_buffer);
...
int32_t SPVM_API_call_method_common(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_RUNTIME_METHOD* method, int32_t args_width, int32_t mortal, int32_t check_args_level, const char* func_name, const char* file, int32_t line) {
  ...
  if (!(check_args_level == SPVM_API_C_CALL_METHOD_CHECK_ARGS_LEVEL_NO_CHECK)) {
    ...
    if (!(check_args_level == SPVM_API_C_CALL_METHOD_CHECK_ARGS_LEVEL_NO_CHECK)) {
      ...
      for (; arg_index < method->args_length; arg_index++) {
        ...
        int32_t arg_stack_index = arg->stack_index;
        if (arg_stack_index < args_width) {
          int32_t arg_is_object_type = SPVM_API_is_object_type((SPVM_RUNTIME*)env->runtime, arg->basic_type, arg->type_dimension, arg->type_flag);
          if (arg_is_object_type) {
            SPVM_OBJECT* obj_arg = (SPVM_OBJECT*)stack[arg_stack_index].oval;
            ...
            if (obj_arg) {
              ...
              if (!can_assign) {
                ...
                error_id = SPVM_API_die(env, stack, "The %ith argument must be assigned to the type of %ith argument of %s#%s method.", __func__, FILE_NAME, __LINE__, arg_index_nth, arg_index_nth, current_basic_type->name, method->name);
                goto END_OF_FUNC;
              }
            }
          }
  ...
  if (method->is_native) {
    ...
    if (method_precompile_address) {
      ...
      error_id = SPVM_API_die(env, stack, "The execution address of %s#%s precompile method must not be NULL. Loading the dynamic link library maybe failed.", __func__, FILE_NAME, __LINE__, method->current_basic_type->name, method->name);
      goto END_OF_FUNC;
    }
  }
  else {
  ...
  if (__builtin_expect(mortal, 0)) {
    ...
    if (method_return_type_is_object) {
      SPVM_API_push_mortal(env, stack, (SPVM_OBJECT*)stack[0].oval);
    }
  }
...
int32_t SPVM_API_call_method_native(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_RUNTIME_METHOD* method, int32_t args_width) {
  ...
  if (__builtin_expect(!native_address, 0)) {
    error_id = SPVM_API_die(env, stack, "The execution address of %s#%s native method must not be NULL. Loading the dynamic link library maybe failed.", __func__, FILE_NAME, __LINE__, method->current_basic_type->name, method->name);
    goto END_OF_FUNC;
  }
...
void SPVM_API_call_class_method_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, const char* method_name, int32_t args_width, int32_t* error_id, const char* func_name, const char* file, int32_t line) {
  ...
  *error_id = 0;
  ...
  SPVM_RUNTIME_METHOD* method = SPVM_API_BASIC_TYPE_get_method_by_name((SPVM_RUNTIME*)env->runtime, basic_type, method_name);
  
  if (!method) {
    *error_id = SPVM_API_die(env, stack, "%s#%s method is not found.", func_name, file, line, basic_type_name, method_name);
    return;
  }
  
  if (!method->is_class_method) {
    *error_id = SPVM_API_die(env, stack, "%s#%s method must be a class method.", func_name, file, line, basic_type_name, method_name);
    return;
  }
...
void SPVM_API_call_instance_method_static_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, const char* method_name, int32_t args_width, int32_t* error_id, const char* func_name, const char* file, int32_t line) {
  ...
  *error_id = 0;
  ...
  if (method->is_class_method) {
    *error_id = SPVM_API_die(env, stack, "%s#%s method must be an instance method.", func_name, file, line, basic_type_name, method_name);
    return;
  }
  ...
  if (!SPVM_API_isa(env, stack, object, basic_type, 0)) {
    *error_id = SPVM_API_die(env, stack, "The invocant must be assigned to %s class.", func_name, file, line, basic_type_name);
    return;
  };
...
int32_t SPVM_API_call_init_methods(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  for (int32_t basic_type_id = 0; basic_type_id < basic_types_length; basic_type_id++) {
    SPVM_RUNTIME_BASIC_TYPE* basic_type = SPVM_API_RUNTIME_get_basic_type_by_id((SPVM_RUNTIME*)env->runtime, basic_type_id);
    if (basic_type->init_method) {
      SPVM_RUNTIME_METHOD* init_method = SPVM_API_BASIC_TYPE_get_method_by_index((SPVM_RUNTIME*)env->runtime, basic_type, basic_type->init_method->index);      
...
int8_t SPVM_API_get_class_var_byte_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, const char* class_var_name, int32_t* error_id, const char* func_name, const char* file, int32_t line) {
  ...
  *error_id = 0;
  ...
  SPVM_RUNTIME_CLASS_VAR* class_var = SPVM_API_BASIC_TYPE_get_class_var_by_name((SPVM_RUNTIME*)env->runtime, basic_type, class_var_name);
  if (!class_var) {
    *error_id = SPVM_API_die(env, stack, "$%s#%s class variable is not found.", func_name, file, line, basic_type_name, class_var_name + 1);
    return 0;
  };
...
void SPVM_API_set_field_byte(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, SPVM_RUNTIME_FIELD* field, int8_t value) {
  ...
  *(int8_t*)(GET_DATA_ADDRESS(env, stack, object) + field->offset) = value;
  SPVM_IMPLEMENT_ENABLE_EXISTS_FLAG(env, stack, (SPVM_OBJ*)object, field->exists_offset, field->exists_bit);
}
...
int8_t SPVM_API_get_field_byte_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, const char* field_name, int32_t* error_id, const char* func_name, const char* file, int32_t line) {
  ...
  *error_id = 0;
  ...
  if (!field) {
    ...
    *error_id = SPVM_API_die(env, stack, "%s#%s field is not found in the class or its super classes.", func_name, file, line, basic_type_name, field_name);
    return 0;
  };
...
SPVM_OBJECT* SPVM_API_get_field_object_defined_and_has_pointer_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, const char* field_name, int32_t* error_id, const char* func_name, const char* file_name, int32_t line) {
  ...
  *error_id = 0;
  ...
  SPVM_OBJECT* obj_field = SPVM_API_get_field_object_by_name(env, stack, object, field_name, error_id, func_name, file_name, line);
  
  if (*error_id) { return NULL; }
  
  if (!obj_field) {
    *error_id = SPVM_API_die(env, stack, "%s field must be defined.", func_name, file_name, line, field_name);
    return obj_field;
  }
...
const char* SPVM_API_get_field_string_chars_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, const char* field_name, int32_t* error_id, const char* func_name, const char* file, int32_t line) {
  ...
  *error_id = 0;
  ...
  SPVM_RUNTIME_FIELD* field = SPVM_API_get_field(env, stack, object, field_name);
  if (!field) {
    *error_id = SPVM_API_die(env, stack, "%s field is not found.", func_name, file, line, field_name);
    return NULL;
  };
...
void SPVM_API_free_stack_internal(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  SPVM_OBJECT** mortal_stack = (SPVM_OBJECT**)stack[SPVM_API_C_STACK_INDEX_MORTAL_STACK].address;
  
  if (mortal_stack != NULL) {
    SPVM_API_free_memory_block_for_execution_stack(env, stack, mortal_stack);
    mortal_stack = NULL;
  }
...
void SPVM_API_free_stack(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  SPVM_API_free_stack_internal(env, stack);
  
  SPVM_VALUE** destroy_stack_stack = (SPVM_VALUE**)stack[SPVM_API_C_STACK_INDEX_DESTROY_EXECUTION_STACK_STACK].address;
  if (destroy_stack_stack) {
    for (int32_t i = 0; i < stack[SPVM_API_C_STACK_INDEX_DESTROY_EXECUTION_STACK_STACK_LENGTH].ival; i++) {
      SPVM_API_free_stack_internal(env, destroy_stack_stack[i]);
    }
    
    SPVM_API_free_memory_block_for_execution_stack(env, stack, destroy_stack_stack);
  }
...
int32_t SPVM_API_is_numeric_object(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object) {
  ...
  if (object) {
    is_numeric_object = SPVM_API_is_numeric_object_type((SPVM_RUNTIME*)env->runtime, object->basic_type, object->type_dimension, 0);
  }
...
int32_t SPVM_API_is_object_array(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object) {
  ...
  if (object) {
    ...
    if (object_type_dimension == 0) {
      ...
      int32_t type_flag = 0;
      is_object_array = SPVM_API_is_object_type((SPVM_RUNTIME*)env->runtime, object_basic_type, element_type_dimension, type_flag);
    }
    else if (object_type_dimension > 1) {
      is_object_array = 1;
    }
...
void SPVM_API_warn(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* string, const char* func_name, const char* file, int32_t line) {
  ...
  if (string) {
    if (SPVM_API_is_type_by_name(env, stack, string, "string", 0)) {
      ...
      int32_t scope_id = SPVM_API_enter_scope(env, stack);
      SPVM_OBJECT* obj_type_name = SPVM_API_get_type_name(env, stack, string);
...
SPVM_OBJECT* SPVM_API_new_string_array_no_mortal(SPVM_ENV* env, SPVM_VALUE* stack, int32_t length) {
  ...
  SPVM_RUNTIME_BASIC_TYPE* string_basic_type = SPVM_API_RUNTIME_get_basic_type_by_id((SPVM_RUNTIME*)env->runtime, SPVM_NATIVE_C_BASIC_TYPE_ID_STRING);
...
SPVM_RUNTIME_FIELD* SPVM_API_get_field_static(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, const char* field_name) {
  ...
  SPVM_RUNTIME_FIELD* field = SPVM_API_BASIC_TYPE_get_field_by_name((SPVM_RUNTIME*)env->runtime, basic_type, field_name);
...
int32_t SPVM_API_isa(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, SPVM_RUNTIME_BASIC_TYPE* basic_type, int32_t type_dimension) {
  ...
  if (object == NULL) {
    ...
    if (!basic_type) {
      ...
      if (!isa) {
        isa = SPVM_API_can_assign((SPVM_RUNTIME*)env->runtime, basic_type, type_dimension, 0, object_basic_type, object_type_dimension, 0);
      }
    }
  }
...
int32_t SPVM_API_is_binary_compatible_stack(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (stack[SPVM_API_C_STACK_INDEX_ENV].address == env) {
...
SPVM_OBJECT* SPVM_API_drop_tag_pointers(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object) {
  ...
  object = (SPVM_OBJECT*)SPVM_IMPLEMENT_DROP_TAG_POINTERS((SPVM_OBJ*)object);
...
FILE* SPVM_API_spvm_stdin(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  FILE* spvm_stdin = SPVM_API_RUNTIME_get_spvm_stdin((SPVM_RUNTIME*)env->runtime);
...
FILE* SPVM_API_spvm_stdout(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  FILE* spvm_stdout = SPVM_API_RUNTIME_get_spvm_stdout((SPVM_RUNTIME*)env->runtime);
...
int32_t SPVM_API_check_bootstrap_method(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name) {
  ...
  if (method) {
    ...
    if (is_class_method) {
      ...
      error_id = SPVM_API_die(env, stack, "%s#main method must be a class method.", __func__, FILE_NAME, __LINE__, basic_type_name);
    }
  }
  else {
    error_id = SPVM_API_die(env, stack, "%s#main method must be defined.", __func__, FILE_NAME, __LINE__, basic_type_name);
  }
...
void SPVM_API_print_exception_to_stderr(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  SPVM_OBJECT* obj_exception = SPVM_API_get_exception(env, stack);
  const char* exception = SPVM_API_get_chars(env, stack, obj_exception);
...
int8_t SPVM_API_numeric_object_to_byte(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, int32_t* error_id) {
  ...
  *error_id = 0;
  ...
  int32_t is_numeric_object_type = SPVM_API_is_numeric_object_type((SPVM_RUNTIME*)env->runtime, object->basic_type, object->type_dimension, 0);
...
SPVM_RUNTIME_METHOD* SPVM_API_get_current_method(SPVM_ENV* env, SPVM_VALUE* stack, int32_t level, int32_t* error_id) {
  ...
  *error_id = 0;
  ...
  int32_t call_depth = SPVM_API_get_call_depth(env, stack);
...
int32_t SPVM_API_die(SPVM_ENV* env, SPVM_VALUE* stack, const char* exception_format, const char* func_name, const char* file, int32_t line, ...) {
