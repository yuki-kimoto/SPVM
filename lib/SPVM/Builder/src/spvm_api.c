...
void SPVM_API_set_field_byte_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, const char* field_name, int8_t value, int32_t* error, const char* func_name, const char* file, int32_t line) {
  *error = 0;
  ...
  if (id < 0) {
    *error = 1;
    const char* class_name = object->basic_type_name;
    env->die(env, stack, "The %s field is not found in the %s class or its super class", field_name, class_name, func_name, file, line);
    return;
  }
...
int32_t SPVM_API_has_interface_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* object, const char* interface_basic_type_name) {
  ...
  if (object->type_dimension > 0) {
    ...
    int32_t interface_basic_type_id = SPVM_API_RUNTIME_get_basic_type_id_by_name(env->runtime, interface_basic_type_name);
...
SPVM_OBJECT* SPVM_API_new_stack_trace_raw(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* exception, int32_t current_method_id, int32_t current_line) {
  ...
  SPVM_RUNTIME_METHOD* method = SPVM_API_RUNTIME_get_method(runtime, current_method_id);
  ...
  const char* new_current_line_part = "\n  ";
  ...
  total_length += strlen(new_current_line_part);
  ...
  const char* current_line_part = " line ";
  char current_line_str[20];
  
  sprintf(current_line_str, "%" PRId32, current_line);
  total_length += strlen(current_line_part);
  total_length += strlen(current_line_str);
...
SPVM_OBJECT* SPVM_API_new_stack_trace(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* exception, int32_t current_method_id, int32_t current_line) {
  (void)env;
  
  SPVM_OBJECT* stack_trace = SPVM_API_new_stack_trace_raw(env, stack, exception, current_method_id, current_line);
...
SPVM_OBJECT* SPVM_API_new_stack_trace_raw_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* exception, const char* current_class_name, const char* current_method_name, int32_t current_line) {
  ...
  int32_t class_id = SPVM_API_RUNTIME_get_class_id_by_name(runtime, current_class_name);
  ...
  total_length += exception_length;
  total_length += strlen(new_current_line_part);
  total_length += strlen(current_class_name);
  total_length += strlen(arrow_part);
  total_length += strlen(current_method_name);
  total_length += strlen(at_part);
  total_length += strlen(class_path);
  total_length += strlen(class_path_sep);
  total_length += strlen(class_rel_file);
...
SPVM_OBJECT* SPVM_API_new_stack_trace_by_name(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* exception, const char* current_class_name, const char* current_method_name, int32_t current_line) {
  (void)env;
  
  SPVM_OBJECT* stack_trace = SPVM_API_new_stack_trace_raw_by_name(env, stack, exception, current_class_name, current_method_name, current_line);
...
SPVM_OBJECT* SPVM_API_new_object_array_raw_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, int32_t length) {
  
  SPVM_RUNTIME* runtime = env->runtime;
  
  int32_t class_id = SPVM_API_RUNTIME_get_class_id_by_name(env->runtime, basic_type_name);
...
SPVM_OBJECT* SPVM_API_new_object_array_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, int32_t length) {
  
  SPVM_OBJECT* object = SPVM_API_new_object_array_raw_by_name(env, stack, basic_type_name, length);
...
SPVM_OBJECT* SPVM_API_new_muldim_array_raw_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, int32_t type_dimension, int32_t length) {
  
  if (type_dimension < 2) {
    return NULL;
  }
  else if (strcmp(basic_type_name, "object") == 0) {
    return NULL;
  }
...
SPVM_OBJECT* SPVM_API_new_muldim_array_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, int32_t type_dimension, int32_t length) {
  
  SPVM_OBJECT* object = SPVM_API_new_muldim_array_raw_by_name(env, stack, basic_type_name, type_dimension, length);
...
SPVM_OBJECT* SPVM_API_new_mulnum_array_raw_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, int32_t length) {
...
SPVM_OBJECT* SPVM_API_new_mulnum_array_by_name(SPVM_ENV* env, SPVM_VALUE* stack, const char* basic_type_name, int32_t length) {
  
  SPVM_OBJECT* object = SPVM_API_new_mulnum_array_raw_by_name(env, stack, basic_type_name, length);
