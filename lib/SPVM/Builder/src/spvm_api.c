...
SPVM_OBJECT* SPVM_API_dump_raw(SPVM_ENV* env, SPVM_OBJECT* object) {
  ...
  int32_t depth = 0;
  SPVM_STRING_BUFFER* string_buffer = SPVM_STRING_BUFFER_new(compiler, 255, SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_RUN_TIME);
...
void* SPVM_API_alloc_memory_block_zero(SPVM_ENV* env, int64_t byte_size) {
  ...
  void* block = SPVM_ALLOCATOR_new_block_runtime(compiler, (size_t)byte_size);
...
#endif
    SPVM_ALLOCATOR_free_block_runtime(compiler, block);
  }
}
