      ...
      for (int32_t array_index = 0; array_index < array_length; array_index++) {
        ...
        if (SPVM_API_is_mulnum_array(env, stack, object)) {
          ...
          for (int32_t field_index = 0; field_index < fields_length; field_index++) {
            ...
            const char* field_name = SPVM_API_RUNTIME_get_global_constant_string_value(runtime, field->name_global_string_address_id, NULL);
...
SPVM_OBJECT* SPVM_API_new_stack_trace_raw(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_OBJECT* exception, SPVM_RUNTIME_METHOD* method, int32_t line) {
  ...
  const char* basic_type_name = SPVM_API_RUNTIME_get_global_constant_string_value(runtime, basic_type->name_global_string_address_id, NULL);
  ...
  if (module_dir_global_string_address_id >= 0) {
    module_dir = SPVM_API_RUNTIME_get_global_constant_string_value_nolen(runtime, module_dir_global_string_address_id);
    module_dir_sep = "/";
  }
  else {
    module_dir = "";
...
const char* SPVM_API_get_version_string(SPVM_ENV* env, SPVM_VALUE* stack, int32_t basic_type_id){
  ...
  if (basic_type->version_string_global_string_address_id >= 0) {
    version_string = SPVM_API_RUNTIME_get_global_constant_string_value_nolen(env->runtime, basic_type->version_string_global_string_address_id);
  }
