...
SPVM_RUNTIME_CLASS* SPVM_API_get_runtime_class_from_basic_type_id(SPVM_ENV* env, int32_t basic_type_id) {
  ...
  if (basic_type_id >= 0) {
    ...
    class = SPVM_LIST_fetch(compiler->runtime_classes, class_id);
  }
...
int32_t SPVM_API_init_env(SPVM_ENV* env) {
  ...
  void* class_vars_heap = calloc(sizeof(SPVM_VALUE), ((int64_t)compiler->runtime_class_vars->length + 1));
    ...
    switch (opcode_id) {
      ...
      case SPVM_OPCODE_C_ID_CALL_INSTANCE_METHOD_BY_SIGNATURE:
      {
        ...
        if (call_method_id < 0) {
          ...
          SPVM_RUNTIME_CLASS* decl_method_class = SPVM_LIST_fetch(compiler->runtime_classes, decl_method->class_id);
      ...
      case SPVM_OPCODE_C_ID_GET_FIELD_BYTE: {
        ...
        SPVM_RUNTIME_FIELD* field = SPVM_LIST_fetch(compiler->runtime_fields, field_id);
        int32_t field_offset = field->offset;
...
SPVM_OBJECT* SPVM_API_new_pointer_raw(SPVM_ENV* env, int32_t basic_type_id, void* pointer) {
  ...
  *(void**)((intptr_t)object + (intptr_t)env->object_header_byte_size) = pointer;
  ...
  // Has destructor
  if (SPVM_LIST_fetch(compiler->runtime_methods, class->method_destructor_id)) {
    object->flag |= SPVM_OBJECT_C_FLAG_HAS_DESTRUCTOR;
  }
