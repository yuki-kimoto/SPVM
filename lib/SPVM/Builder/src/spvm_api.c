...
int32_t SPVM_API_is_object_array(SPVM_ENV* env, SPVM_OBJECT* object) {
  ...
  if (object) {
    is_object_array = object->type_category == SPVM_TYPE_C_TYPE_CATEGORY_OBJECT_ARRAY || object->type_category == SPVM_TYPE_C_TYPE_CATEGORY_ELEMENT_ARRAY;
  }
  else {
    is_object_array = 0;
  }
...
void SPVM_API_dec_ref_count(SPVM_ENV* env, SPVM_OBJECT* object) {
  ...
  if (object->ref_count == 1) {
    ...
    if (object->type_category == SPVM_TYPE_C_TYPE_CATEGORY_OBJECT_ARRAY || object->type_category == SPVM_TYPE_C_TYPE_CATEGORY_ELEMENT_ARRAY) {
      int32_t length = object->length;
      for (int32_t index = 0; index < length; index++) {
