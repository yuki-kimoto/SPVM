...
SPVM_COMPILER_STRING_BUFFER* SPVM_API_STRING_BUFFER_new_instance(SPVM_ALLOCATOR* allocator, int32_t capacity) {
  SPVM_COMPILER_STRING_BUFFER* string_buffer = SPVM_COMPILER_STRING_BUFFER_new_tmp(allocator, capacity);
  
  return string_buffer;
}

const char* SPVM_API_STRING_BUFFER_get_string(SPVM_COMPILER_STRING_BUFFER* string_buffer) {
  return string_buffer->string;
}

int32_t SPVM_API_STRING_BUFFER_get_length(SPVM_COMPILER_STRING_BUFFER* string_buffer) {
  return string_buffer->length;
}

void SPVM_API_STRING_BUFFER_free_instance(SPVM_COMPILER_STRING_BUFFER* string_buffer) {
  SPVM_COMPILER_STRING_BUFFER_free(string_buffer);
}
