...
int32_t SPVM_BASIC_TYPE_has_interface_v2(SPVM_COMPILER* compiler, int32_t basic_type_id, int32_t interface_basic_type_id) {
  ...
  for (int32_t interface_method_index = 0; interface_method_index < interface_basic_type->methods->length; interface_method_index++) {
    ...
    for (int32_t method_index = 0; method_index < basic_type->methods->length; method_index++) {
      ...
      int32_t interface_method_compatibility = SPVM_BASIC_TYPE_check_interface_method_compatibility(compiler, basic_type, method, interface_basic_type, interface_method);
...
int32_t SPVM_BASIC_TYPE_check_interface_method_compatibility(SPVM_COMPILER* compiler, SPVM_BASIC_TYPE* basic_type, SPVM_METHOD* method, SPVM_BASIC_TYPE* interface_basic_type, SPVM_METHOD* interface_method) {
  ...
  if (interface_method->is_required && !method) {
    if (!interface_method->is_class_method) {
      SPVM_COMPILER_error(compiler, "The \"%s\" class must implement the \"%s\" method. This is defined as an interface method in the \"%s\" interface.\n  at %s line %d", interface_method->name, basic_type->name, interface_basic_type->name, basic_type->op_module->file, basic_type->op_module->line);
      return 0;
    }
  }
  ...
  if (method) {
    ...
    for (int32_t arg_index = 1; arg_index < interface_method->args_length; arg_index++) {
      ...
      int32_t assignability_for_method = SPVM_TYPE_can_assign_for_method_definition(compiler, method_var_decl_type->basic_type->id, method_var_decl_type->dimension, method_var_decl_type->flag, interface_method_var_decl_type->basic_type->id, interface_method_var_decl_type->dimension, interface_method_var_decl_type->flag);
