  ...
  // Instance method call
    ...
    // Instance method
      ...
      if (found_method) {
        if (found_method->is_class_method) {
          class = found_method->class;
          SPVM_COMPILER_error(compiler, "The \"%s\" method in the \"%s\" class is found, but this is not an instance method.\n  at %s line %d", method_name, class->type->basic_type->name, op_call_method->file, op_call_method->line);
          return;
        }
        call_method->method = found_method;
      }
...
void SPVM_AST_CHECKER_resolve_field_offset(SPVM_COMPILER* compiler, SPVM_CLASS* class) {
  if (class->type->basic_type->category != SPVM_NATIVE_C_BASIC_TYPE_CATEGORY_CLASS) {
    return;
  }
  ...
  class->type->basic_type->fields_size = offset;
...
void SPVM_AST_CHECKER_resolve_classes(SPVM_COMPILER* compiler) {
  ...
  for (int32_t basic_type_index = compiler->cur_basic_type_base; basic_type_index < compiler->basic_types->length; basic_type_index++) {
    ...
    if (class->type->basic_type->category == SPVM_NATIVE_C_BASIC_TYPE_CATEGORY_MULNUM) {
      ...
      int32_t field_index;
      for (field_index = 0; field_index < class->type->basic_type->fields->length; field_index++) {
      ...
      if (!found_pos_ptr) {
        SPVM_COMPILER_error(compiler, "The type name for the %s multi-numeric with the field length of %d must end with \"%s\".\n  at %s line %d", first_field_type->basic_type->name, class->type->basic_type->fields->length, tail_name, class->op_class->file, class->op_class->line);
        return;
      }
    }
    ...
    for (int32_t i = 0; i < class->type->basic_type->methods->length; i++) {
      ...
      for (int32_t arg_index = 0; arg_index < method->args_length; arg_index++) {
        ...
        if (is_arg_type_is_mulnum_type || is_arg_type_is_value_ref_type) {
          args_stack_length += arg_type->basic_type->class->type->basic_type->fields->length;
        }
        else {
          args_stack_length++;
        }
