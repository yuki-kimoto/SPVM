...
void SPVM_AST_CHECKER_resolve_op_types(SPVM_COMPILER* compiler) {
  ...
  for (int32_t i = 0; i < op_types->length; i++) {
    ...
    if (type->basic_type->id >= SPVM_NATIVE_C_BASIC_TYPE_ID_BYTE_CLASS) {
      ...
      if (!found_class) {
        const char* not_found_class_class_name = SPVM_HASH_get(compiler->not_found_class_name_symtable, basic_type_name, strlen(basic_type_name));
...
void SPVM_AST_CHECKER_traverse_ast_check_syntax(SPVM_COMPILER* compiler, SPVM_CLASS* class, SPVM_METHOD* method) {
  ...
  while (op_cur) {
    ...
    if (op_cur->id == SPVM_OP_C_ID_IF_REQUIRE) {
      ...
      const char* not_found_class_class_name = SPVM_HASH_get(compiler->not_found_class_name_symtable, use_class_name, strlen(use_class_name));
      if (not_found_class_class_name) {
