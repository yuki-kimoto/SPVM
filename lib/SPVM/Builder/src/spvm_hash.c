...
SPVM_HASH* SPVM_HASH_new(SPVM_COMPILER* compiler, int32_t table_capacity, int32_t memory_block_type) {
  ...
  if (memory_block_type == SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_COMPILE_TIME_ETERNAL) {
    ...
    hash = SPVM_ALLOCATOR_new_block_tmp(compiler, sizeof(SPVM_HASH));
  }
  ...
  if (memory_block_type == SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_COMPILE_TIME_ETERNAL) {
    ...
    hash->table = SPVM_ALLOCATOR_new_block_tmp(compiler, hash->table_capacity * sizeof(int32_t));
  }
  ...
  if (memory_block_type == SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_COMPILE_TIME_ETERNAL) {
    ...
    hash->entries =  SPVM_ALLOCATOR_new_block_tmp(compiler, hash->entries_capacity * sizeof(SPVM_HASH_ENTRY));
  }
  ...
  if (memory_block_type == SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_COMPILE_TIME_ETERNAL) {
    ...
    hash->key_buffer = SPVM_ALLOCATOR_new_block_tmp(compiler, hash->key_buffer_capacity);
  }
...
void SPVM_HASH_maybe_extend_entries(SPVM_HASH* hash) {
  ...
  if (entries_length >= entries_capacity) {
    ...
    if (hash->memory_block_type == SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_COMPILE_TIME_ETERNAL) {
      ...
      new_entries = SPVM_ALLOCATOR_new_block_tmp(compiler, new_entries_capacity * sizeof(SPVM_HASH_ENTRY));
    }
...
void SPVM_HASH_maybe_extend_key_buffer(SPVM_HASH* hash, int32_t length) {
  ...
  if (key_buffer_length + length + (int32_t)sizeof(int32_t) >= key_buffer_capacity) {
    ...
    if (hash->memory_block_type == SPVM_COMPIER_ALLOCATOR_C_MEMORY_BLOCK_TYPE_COMPILE_TIME_ETERNAL) {
      ...
      new_key_buffer = SPVM_ALLOCATOR_new_block_tmp(compiler, new_key_buffer_capacity);
    }
