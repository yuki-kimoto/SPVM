...
SPVM_OP* SPVM_OP_build_class(SPVM_COMPILER* compiler, SPVM_OP* op_class, SPVM_OP* op_type, SPVM_OP* op_block, SPVM_OP* op_list_attributes, SPVM_OP* op_extends) {
  ...
  if (!class->is_anon) {
    ...
    if (strcmp(class_name, compiler->cur_rel_file_class_name) != 0) {
      ...
      const char* not_found_class_class_name = SPVM_HASH_get(compiler->not_found_class_symtable, class_name, strlen(class_name));
...
SPVM_OP* SPVM_OP_build_foreach_statement(SPVM_COMPILER* compiler, SPVM_OP* op_for, SPVM_OP* op_var_element, SPVM_OP* op_array, SPVM_OP* op_block_statements) {
  ...
  */
  ...
  // my $.i
  SPVM_OP* op_var_init_for_decl = SPVM_OP_new_op_var_clone(compiler, op_var_init_orig);
  ...
  // my $.array
  SPVM_OP* op_var_array_for_assign = SPVM_OP_new_op_var_clone(compiler, op_var_array_orig);
  ...
  // @$.array
  SPVM_OP* op_var_array_for_length = SPVM_OP_new_op_var_clone(compiler, op_var_array_orig);
  SPVM_OP* op_array_length = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_ARRAY_LENGTH, compiler->cur_file, compiler->cur_line);
  SPVM_OP_build_array_length(compiler, op_array_length, op_var_array_for_length);
  
  // my $.array_length
  SPVM_OP* op_var_array_length_for_assign = SPVM_OP_new_op_var_clone(compiler, op_var_array_length_orig);
  ...
  // $.i < $array_length
  SPVM_OP* op_var_init_for_condition = SPVM_OP_new_op_var_clone(compiler, op_var_init_orig);
  SPVM_OP* op_var_array_length_for_condition = SPVM_OP_new_op_var_clone(compiler, op_var_array_length_orig);
  ...
  // $.i
  SPVM_OP* op_var_init_for_increament = SPVM_OP_new_op_var_clone(compiler, op_var_init_orig);
...
SPVM_OP* SPVM_OP_build_update_op(SPVM_COMPILER* compiler, SPVM_OP* op_update, SPVM_OP* op_access, SPVM_OP* op_diff_value) {
  ...
  if (
    ...
    && (op_access->id == SPVM_OP_C_ID_VAR || op_access->id == SPVM_OP_C_ID_EXCEPTION_VAR || op_access->id == SPVM_OP_C_ID_DEREF || op_access->id == SPVM_OP_C_ID_ARRAY_ACCESS))
  {
    /*
      ++$var, ++$VAR, ++$@
      (
    ...
    */
    ...
    SPVM_OP* op_var_old_clone = SPVM_OP_new_op_var_clone(compiler, op_var_old);
    ...
    if (op_access->id == SPVM_OP_C_ID_VAR || op_access->id == SPVM_OP_C_ID_EXCEPTION_VAR) {
    ...
    else if (op_access->id == SPVM_OP_C_ID_ARRAY_ACCESS) {
      SPVM_OP* op_array_access = op_access;
      ...
      SPVM_OP* op_array_access_clone_get = SPVM_OP_new_op_array_access_clone_v2(compiler, op_array_access, op_var_array_get, op_var_index_get);
      ...
      SPVM_OP* op_array_access_clone_set = SPVM_OP_new_op_array_access_clone_v2(compiler, op_array_access, op_var_array_set, op_var_index_set);
      ...
      if (op_invocant->first->id == SPVM_OP_C_ID_ARRAY_ACCESS) {
        SPVM_OP* op_array_access = op_invocant->first;
        ...
        SPVM_OP* op_array_field_access_clone_get = SPVM_OP_new_op_array_field_access_clone_v2(compiler, op_field_access, op_name_field, op_array_access, op_var_array_get, op_var_index_get);
        ...
        SPVM_OP* op_var_index_set = SPVM_OP_new_op_var(compiler, op_name_var_index);
        SPVM_OP* op_var_index_set_decl = SPVM_OP_new_op_var_decl(compiler, op_index->file,op_index->line);
        SPVM_OP* op_array_field_access_clone_set = SPVM_OP_new_op_array_field_access_clone_v2(compiler, op_array_access, op_name_field, op_array_access, op_var_array_set, op_var_index_set);
        SPVM_OP* op_access_clone_set = op_array_field_access_clone_set;
        SPVM_OP_build_assign(compiler, op_assign_update, op_access_clone_set, op_culc);
      }
      else {
    ...
    if (op_update->id == SPVM_OP_C_ID_POST_INC || op_update->id == SPVM_OP_C_ID_POST_DEC) {
      SPVM_OP* op_var_old_clone_ret = SPVM_OP_new_op_var_clone(compiler, op_var_old_clone);
...
SPVM_OP* SPVM_OP_new_op_var_clone_var_or_assign(SPVM_COMPILER* compiler, SPVM_OP* original_op_var_or_assign) {
  ...
  SPVM_OP* op_var = SPVM_OP_new_op_var_clone(compiler, original_op_var);
  return op_var;
}
...
SPVM_OP* SPVM_OP_new_op_var_tmp(SPVM_COMPILER* compiler, SPVM_TYPE* type, const char* file, int32_t line) {
  ...
  sprintf(name, "$.tmp_%d", compiler->cur_tmp_vars_length);
...
SPVM_OP* SPVM_OP_new_op_name_tmp_var(SPVM_COMPILER* compiler, const char* file, int32_t line) {
  ...
  sprintf(name, "$.op%d", compiler->cur_tmp_vars_length);
  compiler->cur_tmp_vars_length++;
  SPVM_OP* op_name = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_NAME, file, line);
  op_name->uv.name = name;
