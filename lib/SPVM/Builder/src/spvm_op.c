...
SPVM_OP* SPVM_OP_build_class(SPVM_COMPILER* compiler, SPVM_OP* op_class, SPVM_OP* op_type, SPVM_OP* op_block, SPVM_OP* op_list_descriptors) {
  ...
  if (!op_type) {
    ...
    // Anon class name
    char* name_class_tmp = SPVM_ALLOCATOR_alloc_block_permanent(compiler->allocator, anon_method_class_name_length + 1);
    ...
    while ((op_decl = SPVM_OP_sibling(compiler, op_decl))) {
      ...
      // Class var declarations
        ...
        if (class_var->has_setter) {
          ...
          // }
          SPVM_OP* op_method = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_METHOD, op_decl->file, op_decl->line);
          char* method_name_tmp = SPVM_ALLOCATOR_alloc_block_permanent(compiler->allocator, 4 + strlen(class_var->name) - 1 + 1);
      ...
      // Field declarations
        ...
        if (field->has_setter) {
          ...
          SPVM_OP* op_method = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_METHOD, op_decl->file, op_decl->line);
          char* method_name_tmp = SPVM_ALLOCATOR_alloc_block_permanent(compiler->allocator, 4 + strlen(field->name) + 1);
          memcpy(method_name_tmp, "set_", 4);
      ...
      for (i = 0; i < class->methods->length; i++) {
        ...
        // Unknown sub
          ...
          if (found_method_name) {
            ...
            char* method_abs_name = SPVM_ALLOCATOR_alloc_block_permanent(compiler->allocator, method_abs_name_length + 1);
...
SPVM_OP* SPVM_OP_new_op(SPVM_COMPILER* compiler, int32_t id, const char* file, int32_t line) {
  ...
  SPVM_OP *op = SPVM_ALLOCATOR_alloc_block_tmp(compiler->allocator, sizeof(SPVM_OP));
