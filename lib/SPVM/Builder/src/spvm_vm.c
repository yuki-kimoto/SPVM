  ...
  while (1) {
    ...
    switch (opcode_id) {
      ...
      case SPVM_OPCODE_C_ID_WARN: {
        ...
        int32_t include_dir_id = current_basic_type->module_dir_string_address_id;
        if (include_dir_id >= 0) {
          include_dir_sep = "/";
          include_dir = SPVM_API_RUNTIME_get_global_constant_string_value(runtime, current_basic_type->module_dir_string_address_id, NULL);
        }
        else {
          include_dir_sep = "";
          include_dir = "";
        }
        const char* module_rel_file = SPVM_API_RUNTIME_get_global_constant_string_value(runtime, current_basic_type->module_rel_file_string_address_id, NULL);
