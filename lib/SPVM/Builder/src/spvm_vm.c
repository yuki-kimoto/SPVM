...
int32_t SPVM_VM_call_method(SPVM_ENV* env, SPVM_VALUE* stack, SPVM_RUNTIME_METHOD* current_method, int32_t items) {
  ...
  int32_t object_header_size = (intptr_t)env->api->runtime->get_object_header_size(env->runtime);
  int32_t object_ref_count_offset = (intptr_t)env->api->runtime->get_object_ref_count_offset(env->runtime);
