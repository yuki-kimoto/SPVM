...
SPVM_STRING_BUFFER* SPVM_STRING_BUFFER_new(SPVM_COMPILER* compiler, int32_t capacity) {
  ...
  SPVM_STRING_BUFFER* string_buffer = (SPVM_STRING_BUFFER*)SPVM_ALLOCATOR_new_block_tmp(compiler, sizeof(SPVM_STRING_BUFFER));
  
  string_buffer->capacity = capacity;
  string_buffer->buffer = (char*)SPVM_ALLOCATOR_new_block_tmp(compiler, capacity);
...
void SPVM_STRING_BUFFER_maybe_extend(SPVM_STRING_BUFFER* string_buffer, int32_t new_length) {
  ...
  while (new_length > string_buffer->capacity) {
    ...
    char* new_buffer = SPVM_ALLOCATOR_new_block_tmp(compiler, new_capacity);
