...
int32_t SPVM__Native__MethodCall__new_instance_method(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  const char* invocant = env->get_chars(env, stack, obj_invocant);
  ...
  void* method = env->get_instance_method(env, stack, invocant, method_name);
  if (!method) {
    return env->die(env, stack, "The \"%s\" method in the \"%s\" class cannot be found.", method_name, invocant, __func__, FILE_NAME, __LINE__);
  }
