  ...
  while (1) {
    ...
    switch (opcode->id) {
      ...
      case SPVM_OPCODE_C_ID_PUSH_MORTAL: {
        assert(mortal_stack_top < runtime_sub->mortal_stack_length);
      ...
      {
        int32_t rel_id = opcode->operand1;
        int32_t decl_sub_id = (intptr_t)SPVM_LIST_fetch(runtime->info_sub_ids, runtime_sub->info_sub_ids_base + rel_id);
