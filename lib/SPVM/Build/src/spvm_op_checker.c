        ...
        for (sub_index = 0; sub_index < op_subs->length; sub_index++) {
          ...
          
          {
            ...
            for (arg_index = 0 ; arg_index < sub->op_args->length; arg_index++) {
              ...
              SPVM_TYPE* arg_type = SPVM_OP_get_type(compiler, op_arg);
              if (SPVM_TYPE_is_object_type(compiler, arg_type)) {
                      ...
                      case SPVM_OP_C_ID_POST_INC:
                      {
                        ...
                        if (!op_cur->is_assigned_to_var && !op_cur->is_passed_to_sub) {
                          ...
                          //   VAR_RET
                            ...
                            SPVM_OP* op_var_inc = SPVM_OP_new_op_var(compiler, op_var->uv.var->op_name);
                            op_var_inc->uv.var->op_my = op_var->uv.var->op_my;
                            
                            SPVM_OP* op_inc = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_INC, op_cur->file, op_cur->line);
                            SPVM_OP_insert_child(compiler, op_inc, op_inc->last, op_var_inc);
                            
                            SPVM_OP* op_var_ret = SPVM_OP_new_op_var(compiler, op_var_tmp->uv.var->op_name);
                            op_var_ret->uv.var->op_my = op_var_tmp->uv.var->op_my;
                      ...
                      case SPVM_OP_C_ID_POST_DEC:
                      {
                        ...
                        if (!op_cur->is_assigned_to_var && !op_cur->is_passed_to_sub) {
                          ...
                          if (op_cur->id == SPVM_OP_C_ID_PRE_DEC) {
                            ...
                            SPVM_OP* op_sequence = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_SEQUENCE, op_cur->file, op_cur->line);
                            SPVM_OP* op_var_dec = SPVM_OP_new_op_var(compiler, op_var->uv.var->op_name);
                            op_var_dec->uv.var->op_my = op_var->uv.var->op_my;
                            ...
                            SPVM_OP* op_var_ret = SPVM_OP_new_op_var(compiler, op_var->uv.var->op_name);
                            op_var_ret->uv.var->op_my = op_var->uv.var->op_my;
                          ...
                          //   VAR_RET
                            ...
                            SPVM_OP* op_sequence = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_SEQUENCE, op_cur->file, op_cur->line);
                            SPVM_OP* op_var_from = SPVM_OP_new_op_var_from_op_my(compiler, op_var->uv.var->op_my, op_cur->file, op_cur->line);
                            ...
                            SPVM_OP* op_var_dec = SPVM_OP_new_op_var_from_op_my(compiler, op_var->uv.var->op_my, op_cur->file, op_cur->line);
                            
                            SPVM_OP* op_dec = SPVM_OP_new_op(compiler, SPVM_OP_C_ID_DEC, op_cur->file, op_cur->line);
                            SPVM_OP_insert_child(compiler, op_dec, op_dec->last, op_var_dec);
                            
                            SPVM_OP* op_var_ret = SPVM_OP_new_op_var_from_op_my(compiler, op_var_tmp->uv.var->op_my, op_cur->file, op_cur->line);
                            SPVM_OP_insert_child(compiler, op_sequence, op_sequence->last, op_assign);
                            SPVM_OP_insert_child(compiler, op_sequence, op_sequence->last, op_dec);
                            SPVM_OP_insert_child(compiler, op_sequence, op_sequence->last, op_var_ret);
                  ...
                  if (!op_cur->is_lvalue && !op_cur->is_assigned_to_var) {
                    switch (op_cur->id) {
                      ...
                      case SPVM_OP_C_ID_ARRAY_FIELD_ACCESS:
                        ...
                        break;
                      case SPVM_OP_C_ID_CONSTANT: {
                        if (SPVM_TYPE_is_numeric_type(compiler, tmp_var_type) && op_cur->flag != SPVM_OP_C_FLAG_CONSTANT_CASE) {
                          create_tmp_var = 1;
                        }
                        break;
                      }
...
SPVM_OP* SPVM_OP_CHECKER_check_and_convert_type(SPVM_COMPILER* compiler, SPVM_OP* op_assign_to, SPVM_OP* op_assign_from) {
  ...
  if (SPVM_TYPE_is_numeric_type(compiler, assign_to_type) && op_assign_from->id == SPVM_OP_C_ID_UNDEF) {
    SPVM_yyerror_format(compiler, "Can't convert undef to numeric type at %s line %d\n", op_assign_to->file, op_assign_to->line);
  }
  else {
    // Numeric type check
    if (SPVM_TYPE_is_numeric_type(compiler, assign_to_type) && SPVM_TYPE_is_numeric_type(compiler, assign_from_type)) {
      ...
      if ((assign_to_type->dimension == 1 && assign_to_type->basic_type->id == SPVM_BASIC_TYPE_C_ID_BYTE) && SPVM_TYPE_is_numeric_type(compiler, assign_from_type)) {
  ...
  for (package_index = 0; package_index < compiler->op_packages->length; package_index++) {
    ...
    if (package->category == SPVM_PACKAGE_C_CATEGORY_VALUE_T) {
      ...
      // Max fields length is 16
        ...
        if (!SPVM_TYPE_is_numeric_type(compiler, first_field_type)) {
    ...
    // Check fields
    {
      ...
      for (field_index = 0; field_index < op_package->uv.package->op_fields->length; field_index++) {
        ...
        _Bool is_value_t = SPVM_TYPE_is_value_type(compiler, field_type);
        if (is_value_t) {
          ...
          if (SPVM_TYPE_is_object_type(compiler, field->op_type->uv.type)) {
    ...
    // valut_t can't become package variable
    {
      ...
      for (package_var_index = 0; package_var_index < op_package->uv.package->op_package_vars->length; package_var_index++) {
        ...
        _Bool is_value_t = SPVM_TYPE_is_value_type(compiler, package_var_type);
