...
sub get_dynamic_lib_file_dist {
  ...
  my $module_module_file = $self->get_module_file($self->runtime, $class_name);
...
sub build_dynamic_lib_dist {
  ...
  my $method_names = $self->get_method_names($self->runtime, $class_name, $category);
...
sub bind_methods {
  ...
  if (@$method_names) {
    ...
    if ($category eq 'precompile') {
      my $anon_class_names = $self->get_anon_class_names_by_parent_class_name($self->runtime, $class_name);
...
sub build_precompile_class_source_file {
  ...
  my $module_file = $self->get_module_file($runtime, $class_name);
