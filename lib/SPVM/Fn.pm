...
Parses the C<UTF-8> character at the value reffered by $offset_ref of the $string and return its Unicode code point.
...
If the $length is less than C<0>, the $length is set to the length of the $string - the $offset.
...
If the $limit is less than C<0>, split the $string by the specific $separator and convert them to an string array and return it.
...
Converts the $string to the C<int> value with $digit using C<strtol> in C<C language>.
...
Converts the $string to the C<long> value with $digit using C<strtoll> in C<C language>.
