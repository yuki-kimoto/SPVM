...
int32_t SPNATIVE__SPVM__Socket__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (handle < 0) {
    SPVM_DIE("Can't create socket at %s line %d", "SPVM/Socket.c", __LINE__);
  }
  ...
  if (server.sin_addr.s_addr == 0xffffffff) {
    ...
    if (host == NULL) {
      if (h_errno == HOST_NOT_FOUND) {
        ...
        SPVM_DIE("Exception at %s line %d", "SPVM/Socket.c", __LINE__);
      }
      return 1;
    }
    ...
    if (*addrptr == NULL) {
      SPVM_DIE("Connect at %s line %d", "SPVM/Socket.c", __LINE__);
    }
  }
...
int32_t SPNATIVE__SPVM__Socket__write(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (write_length < 0) {
    SPVM_DIE("Socket write error at %s line %d", "SPVM/Socket.c", __LINE__);
  }
...
int32_t SPNATIVE__SPVM__Socket__read(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (read_length < 0) {
    SPVM_DIE("Socket read error at %s line %d", "SPVM/Socket.c", __LINE__);
  }
