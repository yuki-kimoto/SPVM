...
# Call subroutine
{
  ok(TestCase::Math->sin());
  ok(TestCase::Math->cos());
  ok(TestCase::Math->tan());
}

# float
{
  ok(TestCase::Math->float_pass_positive_infinity($POSITIVE_INFINITY));
  ok(TestCase::Math->float_pass_nan($NaN));
  
  ok(TestCase::Math->isinff());
  ok(TestCase::Math->isfinitef());
  ok(TestCase::Math->isnanf());
...
# SPVM::Double
{
  ok(TestCase::Math->double_pass_positive_infinity($POSITIVE_INFINITY));
  ok(TestCase::Math->double_pass_nan($NaN));
  
  ok(TestCase::Math->isinf());
  ok(TestCase::Math->isfinite());
  ok(TestCase::Math->isnan());
