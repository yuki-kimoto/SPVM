...
int32_t SPVM__TestCase__NativeAPI__native_call_class_method_by_name(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  {
    stack[0].ival = 5;
    int32_t error = env->call_class_method_by_name(env, stack, "TestCase::NativeAPI", "my_value", "int(int)", MFILE, __LINE__);
    if (error) {
      return error;
    }
    output = stack[0].ival;
...
int32_t SPVM__TestCase__NativeAPI__native_call_class_method_by_name_exception(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  {
    stack[0].ival = 5;
    int32_t error = env->call_class_method_by_name(env, stack, "TestCase::NativeAPI", "not_found", "int(int)", MFILE, __LINE__);
    if (error) {
      return error;
    }
    output = stack[0].ival;
...
int32_t SPVM__TestCase__NativeAPI__call_instance_method_by_name_native(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  {
    stack[0].oval = minimal;
    int32_t error = env->call_instance_method_by_name(env, stack, minimal, "x", "int(self)", MFILE, __LINE__);
    if (e) { return e; }
    output = stack[0].ival;
  }
