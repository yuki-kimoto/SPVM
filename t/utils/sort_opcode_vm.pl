# Sort the spvm func by name asc order
...
while (my $line = <>) {
  ...
  if ($is_func) {
    if ($line =~ /^      case\s+(SPVM_OPCODE_C_ID_\w+?)\s*:/) {
      $cur_name = $1;
      $func_defs_h->{$cur_name} = $line;
    }
    else {
      $func_defs_h->{$cur_name} .= $line;
    }
...
for my $func_name (sort { $order->{$a} <=> $order->{$b} } keys %$func_defs_h) {
  my $func_def = $func_defs_h->{$func_name};
  print $func_def;
}
