...
{
  # new_long_array_from_bin - Return type
  {
  ...
  # new_long_array_from_bin - binary signed
  {
    my $binary = pack('q*', 97, 98, $LONG_MAX);
    my $spvm_array = $api->new_long_array_from_bin($binary);
    my $values = $spvm_array->to_elems;
    is_deeply($values, [97, 98, $LONG_MAX]);
  }
  
  # new_long_array_from_bin - binary unsigned
  {
    my $binary = pack('Q*', 97, 98, $ULONG_MAX);
    my $spvm_array = $api->new_long_array_from_bin($binary);
    my $values = $spvm_array->to_elems;
    is_deeply($values, [97, 98, -1]);
  }
  
  # new_long_array_from_bin - Exceptions
  {
    # undef
    {
...
{
  # new_double_array_from_bin - Return type
  {
    my $binary = pack('d*', 97, 98, $FLOAT_MAX);
    my $spvm_array = $api->new_double_array_from_bin($binary);
    is(ref $spvm_array, 'SPVM::BlessedObject::Array');
    is($spvm_array->__get_type_name, "double[]");
  }
  
  # new_double_array_from_bin - binary
  {
    my $binary = pack('d*', 97, 98, $FLOAT_MAX);
    my $spvm_array = $api->new_double_array_from_bin($binary);
    my $values = $spvm_array->to_elems;
  ...
  # new_double_array_from_bin - Exceptions
  {
    # undef
    {
  ...
  {
    {
      ...
      ok(index($@, 'The type name $type_name was parsed, but the module name could not be extracted') >= 0);
    }
    {
