...
int32_t SPVM__TestCase__NativeAPI__check_native_api_ids(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if ((void*)&env->reserved231 != &env_array[231]) { stack[0].ival = 0; return 0; }
...
int32_t SPVM__TestCase__NativeAPI__check_native_api_compiler_ids(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if ((void*)&env->api->compiler->get_class_search_dirs_length != &env_array[6]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->compiler->get_class_search_dir != &env_array[7]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->compiler->add_class_search_dir != &env_array[8]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->compiler->clear_class_search_dirs != &env_array[9]) { stack[0].ival = 0; return 0; }
  ...
  if ((void*)&env->api->compiler->prepend_class_search_dir != &env_array[17]) { stack[0].ival = 0; return 0; }
...
int32_t SPVM__TestCase__NativeAPI__check_native_api_cfunc_ids(SPVM_ENV* env, SPVM_VALUE* stack) {
  
  int32_t error_id = 0;
  
  void** env_array = (void**)env->api->cfunc;
  
  if ((void*)&env->api->cfunc->c_strlen != &env_array[0]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->cfunc->c_memcpy != &env_array[1]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->cfunc->c_memset != &env_array[2]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->cfunc->c_memcmp != &env_array[3]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->cfunc->c_strtoll != &env_array[4]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->cfunc->c_strtof != &env_array[5]) { stack[0].ival = 0; return 0; }
  if ((void*)&env->api->cfunc->c_strtod != &env_array[6]) { stack[0].ival = 0; return 0; }
...
int32_t SPVM__TestCase__NativeAPI__spvm_warn(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  spvm_diag("[Test Output]spvm_diag: %s %d", "aaa", 3);
  spvm_diag("[Test Output]spvm_diag:Foo");
...
int32_t SPVM__TestCase__NativeAPI__default_all_types_native(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (args_width >= 5) {
    ...
    if (!(value_float == 1.5f)) {
      ...
      spvm_diag("%.20f %.20f", value_float, 1.5f);
...
int32_t SPVM__TestCase__NativeAPI__die(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  
  {
    char* tmp_buffer = env->get_stack_tmp_buffer(env, stack);
    sprintf(tmp_buffer, "%s", "abcd");
    int32_t error_id = env->die(env, stack, "Error %s.", __func__, __FILE__, __LINE__, tmp_buffer);
    ...
    SPVM_OBJ* obj_exception = env->longmess(env, stack, obj_message, __func__, __FILE__, __LINE__, 0); int32_t line = __LINE__;
    const char* exception = env->get_chars(env, stack, obj_exception);
