...
int32_t SPVM_FIELD_get_byte_size(SPVM_COMPILER* compiler, SPVM_FIELD* field) {
  ...
  int32_t byte_size;
  if (field_resolved_type->id == SPVM_TYPE_C_ID_BYTE) {
    byte_size = sizeof(int8_t);
  }
  else if (field_resolved_type->id == SPVM_TYPE_C_ID_SHORT) {
    byte_size = sizeof(int16_t);
  }
  else if (field_resolved_type->id == SPVM_TYPE_C_ID_INT || field_resolved_type->id == SPVM_TYPE_C_ID_FLOAT) {
    byte_size = sizeof(int32_t);
  }
  else if (field_resolved_type->id == SPVM_TYPE_C_ID_LONG || field_resolved_type->id == SPVM_TYPE_C_ID_DOUBLE) {
