...
int32_t SPVM_RUNTIME_ALLOCATOR_get_freelist_index(SPVM_SNI* sni, SPVM_RUNTIME_ALLOCATOR* allocator, int64_t byte_size) {
  (void)sni;
  (void)allocator;
...
void* SPVM_RUNTIME_ALLOCATOR_malloc(SPVM_SNI* sni, SPVM_RUNTIME_ALLOCATOR* allocator, int32_t byte_size) {
  SPVM_RUNTIME* runtime = SPVM_GLOBAL_RUNTIME;
  
  assert(byte_size > 0);
  int32_t index = SPVM_RUNTIME_ALLOCATOR_get_freelist_index(sni, allocator, byte_size);
...
void SPVM_RUNTIME_ALLOCATOR_free_object(SPVM_SNI* sni, SPVM_RUNTIME_ALLOCATOR* allocator, SPVM_OBJECT* object) {
  ...
  if (object == NULL) {
    ...
    // Byte size
    int32_t byte_size = SPVM_RUNTIME_API_calcurate_object_byte_size(sni, object);
    ...
    if (byte_size > allocator->object_max_byte_size_use_memory_pool) {
      ...
      // Freelist index
      int32_t freelist_index = SPVM_RUNTIME_ALLOCATOR_get_freelist_index(sni, allocator, byte_size);
